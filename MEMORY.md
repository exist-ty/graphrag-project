...
- Решение по квоте не принято: варианты — включить биллинг, перевести пайплайн на
  `gemini-3.5-flash-lite` (квота есть, но модель слабее для извлечения сущностей), либо ждать
  суточного сброса. Требуется выбор пользователя.
